...
class CharConverter(TypeConverterBase):
    ...
    def input_conversion(self, cpp_type, argument_var, arg_num):
        cr_ref = False
        code = "py_builtin$bytes(%s,'utf-8')" % (argument_var)
...
class StdPairConverter(TypeConverterBase):
    ...
    def input_conversion(self, cpp_type, argument_var, arg_num):
        ...
        if i1.is_enum:
            ...
            if isinstance(inner_conv1,(CharConverter,ConstCharPtrConverter,CharPtrConverter,StdStringConverter)):
                # code.add("""
        ...
        if i2.is_enum:
            ...
            if isinstance(inner_conv2,(CharConverter,ConstCharPtrConverter,CharPtrConverter,StdStringConverter)):
                # code.add("""
...
class StdMapConverter(TypeConverterBase):
    ...
    def type_check_expression(self, cpp_type, arg_var):
        ...
        #       """, locals()).render()
        # elif isinstance(inner_conv_1,(CharConverter,ConstCharPtrConverter,CharPtrConverter,StdStringConverter)):
        #     return Code().add("""
        #       |is_list($arg_var)
        #       + && ifelse(length($arg_var)==0,TRUE,!is.null(names($arg_var))
    ...
    def input_conversion(self, cpp_type, argument_var, arg_num):
        ...
        if cy_tt_value.is_enum:
            ...
            if self.converters.cython_type(tt).is_enum:
                ...
                if isinstance(inner_conv,IntegerConverter):
                ...
                elif isinstance(inner_conv,(CharConverter,ConstCharPtrConverter,CharPtrConverter,StdStringConverter)):
                    ...
                    if isinstance(conv_t1,IntegerConverter):
                        if isinstance(conv_t2,IntegerConverter):
                            value_conv = "map_depth(%s$values(),%s,function(a) list(as.integer(a[[1]]),as.integer(a[[2]])))" % (
                            argument_var, depth_cnt)
                        elif isinstance(conv_t2,(CharConverter,ConstCharPtrConverter,CharPtrConverter,StdStringConverter)):
                            ...
                            value_conv = "map_depth(%s$values(),%s,function(a) list(as.integer(a[[1]]),a[[2]]))" % (
                                argument_var, depth_cnt)
                    elif isinstance(conv_t2,IntegerConverter):
                        if isinstance(conv_t1,(CharConverter,ConstCharPtrConverter,CharPtrConverter,StdStringConverter)):
                            ...
                            value_conv = "map_depth(%s$values(),%s,function(a) list(a[[1]],as.integer(a[[2]])))" % (
                                argument_var, depth_cnt)
                    elif isinstance(conv_t1,(CharConverter,ConstCharPtrConverter,CharPtrConverter,StdStringConverter)):
                        if isinstance(conv_t2,(CharConverter,ConstCharPtrConverter,CharPtrConverter,StdStringConverter)):
        ...
        if cpp_type.is_ref and not cpp_type.is_const:
            ...
            if tt_key.base_type in self.converters.names_of_wrapper_classes \
              and not tt_value.base_type in self.converters.names_of_wrapper_classes:
                ...
                if isinstance(k_conv,(CharConverter,CharPtrConverter,ConstCharPtrConverter,StdStringConverter,StdStringUnicodeConverter)):
                    cleanup_code = Code().add("""
...
class StdSetConverter(TypeConverterBase):
    ...
    def type_check_expression(self, cpp_type, arg_var):
        ...
        if isinstance(inner_conv,(TypeToWrapConverter)):
            return """
              |is.vector(%s) && all(sapply(%s,function(el) %s)) && length(%s) == py_to_r(py_builtin$len(py_builtin$set(%s)))
    ...
    def output_conversion(self, cpp_type, input_cpp_var, output_py_var):
        ...
        if inner.is_enum:
            ...
            code = Code().add("""
                |$output_py_var <- py_to_r(py_builtin$$list($input_cpp_var))
                """, locals())
            # code = Code().add("""
            #     |$output_py_var = set()
            ...
            # input_cpp_var : input_py_var
            code = Code().add("""
                |$output_py_var <- lapply(py_to_r(py_builtin$$list($input_cpp_var)), function(a) $cy_tt$$new(a))
                """, locals())
            # code = Code().add("""
            #     |$output_py_var = set()
            #     |cdef libcpp_set[$inner].iterator $it = $input_cpp_var.begin()
            ...
            if cpp_type.topmost_is_ref and not cpp_type.topmost_is_const:
                ...
                if isinstance(conv_k,StdStringConverter):
                    cleanup_code = Code().add("""
                        ...
                        """, locals())
                elif k.base_type in ["int","float","double"] :
                    cleanup_code = Code().add("""
                        |byref_${arg_num} <- py_to_r($temp_var)
                        """, locals())
                else:
    ...
    def output_conversion(self, cpp_type, input_r_var, output_r_var):
        ...
        if inner.is_enum:
            ...
            if is_converter_present:
                if isinstance(inner_conv, (CharConverter,ConstCharPtrConverter,CharPtrConverter,StdStringConverter)):
                    code = Code().add("""
